...
//console.log(`Tamaño de la funcion arreglo:${product().length}`)
  ...
  //Envia el carrito de un usuario
  //
  getUsCarrito: async (req, res,next) => {
    ...
    try {
       ...
       let carritos= await api.findAll()
        ...
        const carrito=carritos.find(c =>c.idUsuario==idU)

       
       res.json({
        mensage: `Carrito del Usuario con id:${carrito._id}`,
        carrito:carrito.productos,
      });

    } catch (error) {

       const error1 = new Error(`(get)-No se encuentra el carrito del usuario id: ${idU}`);
       error.httpStatusCode = 400;
  ...
  postProdCarrito: async (req, res,next) => {
    ...
    let producto={...prod[0]._doc,cantidad:Number(cant)}
    ...
    res.json({
      ...
      const error1 = new Error(`(post)-No se pudo poner un producto en el carrito con el id: ${error}`);
      error.httpStatusCode = 400;
      return next(error1);
    }
  },
